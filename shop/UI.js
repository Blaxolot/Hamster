function createBoxes() {
  items.forEach(item => {
    ...
    eval(
      `${item}_box.add([text(price1), scale(0.9), pos(isOneFood ? 90: 55, 13)])`,
    );
    !isOneFood &&
      eval(`${item}_box.add([
      sprite("${second_food}"),
    ...
    if (
      !(item == "mystery" && localStorage.getItem("?*$?r2?") == "$q?q&?a??")
    ) {
...
function createButtons() {
  items.forEach(item => {
    ...
    if (localStorage.getItem(condition.key) == condition.value) {
      ...
      eval(
        `buy_${item}_button_color = status ? rgb(0, 160, 0) : rgb(200, 0, 0)`,
      );
      ...
      eval(
        `buy_${item}_button_color = con ? rgb(0, 200, 0) : rgb(250, 25, 25)`,
      );
